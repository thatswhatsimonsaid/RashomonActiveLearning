...
### CONFIGURATION ###
BASE_STUDY_DIR = PROJECT_ROOT / "results" / "study2_visual_analysis" / "3_continuous_heatmap"
...
def load_and_average_results():
    """
    Loads all 'task_*.pkl' files and averages them.
    Returns a dict: {'QBC-RF': avg_grid, 'UNREAL': avg_grid}
    """
...
def plot_side_by_side_heatmaps(avg_heatmaps, n_sims):
    """
    Generates a 1x2 side-by-side heatmap comparison (QBC-RF vs UNREAL).
    """
    if "QBC-RF" not in avg_heatmaps or "UNREAL" not in avg_heatmaps:
        print("Error: Missing data for QBC-RF or UNREAL.")
        return
    ...
    # 5. Shared Colorbar
    cbar = fig.colorbar(images[0], ax=axes.ravel().tolist(), shrink=0.9, pad=0.05)
