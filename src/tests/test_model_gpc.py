...
### TEST THIS CLASS ###
from src.utils.models import GaussianProcessWrapper
...
### TEST FUNCTIONS ###
def test_gpc_fit_predict(dummy_classification_data):
    """Tests that the GaussianProcessWrapper can fit and predict."""
...
def test_gpc_get_log_probas(dummy_classification_data):
    """Tests the predict_log_proba_ensemble method for GPC."""
    X_train, y_train = dummy_classification_data
    model = GaussianProcessWrapper()
    n_samples = 10
    model.fit(X_train, y_train)
